...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  console.log("[AuthProvider] Component rendered/re-rendered. Initial loading state:", loading, "Pathname:", pathname);

  // Check for existing user session on initial client-side load
  useEffect(() => {
    console.log("[AuthProvider] Mount/Effect to load user triggered. Pathname:", pathname);
    const loadUser = () => {
      ...
      try {
        ...
        console.log("[AuthProvider] loadUser finally block. Setting loading to false.");
    ...
    if (typeof window !== "undefined") {
      console.log("[AuthProvider] Window defined, proceeding to call loadUser().");
      loadUser()
    } else {
      console.log("[AuthProvider] Window undefined (SSR), loadUser() skipped. Loading remains true from server.");
    }
    ...
    if (!loading) {
      if (user && (pathname === "/login" || pathname === "/register")) {
        ...
        if (!publicPaths.includes(pathname)) {
          console.log(`[AuthProvider] No user, and not on a public path or auth page (${pathname}). Redirecting to /login.`);
          router.push("/login");
        }
      }
    }
  ...
  const login = (backendUser: BackendUser) => {
    console.log("[AuthProvider] login function called with backendUser:", backendUser);
    const userToStore: User = {
      id: backendUser.id,
      email: backendUser.email,
      name: `${backendUser.first_name || ""} ${backendUser.surname || ""}`.trim() || backendUser.email,
    ...
    setUser(userToStore)
    console.log("[AuthProvider] User set and stored. Navigating to /dashboard via router.push.");
    router.push("/dashboard")
  }
  ...
  const logout = () => {
    ...
    setUser(null)
    console.log("[AuthProvider] User removed from state and localStorage. Navigating to /login.");
    router.push("/login")
  }
