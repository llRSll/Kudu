import { pgTable, uuid, varchar, text, timestamp, jsonb, numeric, boolean, integer, date, primaryKey } from 'drizzle-orm/pg-core';
...
export const RolePermissions = pgTable('role_permissions', {
  ...
  permission_id: uuid('permission_id').notNull().references(() => DbPermissions.id),
  created_at: timestamp('created_at', { withTimezone: true }),
}, (table) => ({
...
export const FacilityOffsetAccounts = pgTable('facility_offset_accounts', {
  id: uuid('id').primaryKey().defaultRandom(),
  facility_id: uuid('facility_id').references(() => CreditFacilities.id),
  offset_account_id: uuid('offset_account_id').references(() => OffsetAccounts.id),
  created_at: timestamp('created_at', { withTimezone: true }),
  updated_at: timestamp('updated_at', { withTimezone: true }),
});
